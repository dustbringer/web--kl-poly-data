    ...
    Promise.resolve()
      .then(() => {
        if (savedBM.current[bmInv] === undefined) {
          ...
          return Promise.all([
            fetch(
              staticify(`/static/tmp/ballmapper-h-poly/bm-${bmInv}.edge.out`),
            ),
            fetch(
              staticify(`/static/tmp/ballmapper-h-poly/bm-${bmInv}.pcbl.out`),
            ),
          ])
      ...
      .then(() => {
        if (savedBM.current[bmCmpInv] === undefined) {
          ...
          return Promise.all([
            fetch(
              staticify(
                `/static/tmp/ballmapper-h-poly/bm-${bmCmpInv}.edge.out`,
              ),
            ),
            fetch(
              staticify(
                `/static/tmp/ballmapper-h-poly/bm-${bmCmpInv}.pcbl.out`,
              ),
            ),
          ])
            .then((res) => Promise.all(res.map((r) => r.text())))
