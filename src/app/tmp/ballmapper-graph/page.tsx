...
export default function BallmapperPage() {
  ...
  const [bmInv, setBmInv] = React.useState<string>("chPoly-2-9-e=0.3");
  ...
  const [bmCmpInv, setBmCmpInv] = React.useState<string>("chPoly-2-9-e=0.5");
    ...
    Promise.resolve()
      .then(() => {
        if (savedBM.current[bmInv] === undefined) {
          ...
          return Promise.all([
            fetch(
              staticify(`/static/tmp/ballmapper-graph/bm-${bmInv}.edge.out`),
            ),
            fetch(
              staticify(`/static/tmp/ballmapper-graph/bm-${bmInv}.pcbl.out`),
            ),
          ])
    ...
    Promise.resolve()
      .then(() => {
        if (savedBM.current[bmCmpInv] === undefined) {
          ...
          return Promise.all([
            fetch(
              staticify(`/static/tmp/ballmapper-graph/bm-${bmCmpInv}.edge.out`),
            ),
            fetch(
              staticify(`/static/tmp/ballmapper-graph/bm-${bmCmpInv}.pcbl.out`),
            ),
          ])
            .then((res) => Promise.all(res.map((r) => r.text())))
            .then((res) => {
