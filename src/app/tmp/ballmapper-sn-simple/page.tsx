...
const optionsBM: { [name: string]: string } = {
  ["sn-simples-mod2-1-19-3rt-e=0.3"]: "sn-simples-mod2-1-19-3rt-e=0.3",
  ["sn-simples-mod2-1-19-5rt-e=0.3"]: "sn-simples-mod2-1-19-5rt-e=0.3",
  ["sn-simples-mod3-1-19-3rt-e=0.3"]: "sn-simples-mod3-1-19-3rt-e=0.3",
  ["sn-simples-mod3-1-19-5rt-e=0.3"]: "sn-simples-mod3-1-19-5rt-e=0.3",
  ["sn-simples-mod4-1-18-3rt-e=0.3"]: "sn-simples-mod4-1-18-3rt-e=0.3",
  ["sn-simples-mod4-1-18-5rt-e=0.3"]: "sn-simples-mod4-1-18-5rt-e=0.3",
  ["sn-simples-mod5-1-18-3rt-e=0.3"]: "sn-simples-mod5-1-18-3rt-e=0.3",
  ["sn-simples-mod5-1-18-5rt-e=0.3"]: "sn-simples-mod5-1-18-5rt-e=0.3",
};
...
export default function BallmapperPage() {
  ...
  const [bmInv, setBmInv] = React.useState<string>(
    "sn-simples-mod2-1-19-3rt-e=0.3",
  );
      ...
      .then(() => {
        if (savedBM.current[bmInv] === undefined) {
          ...
          return Promise.all([
            fetch(
              staticify(
                `/static/tmp/ballmapper-sn-simples/bm-${bmInv}.edge.out`,
              ),
            ),
            fetch(
              staticify(
                `/static/tmp/ballmapper-sn-simples/bm-${bmInv}.pcbl.out`,
              ),
            ),
          ])
      ...
      .then(() => {
        if (savedBM.current[bmCmpInv] === undefined) {
          ...
          return Promise.all([
            fetch(
              staticify(
                `/static/tmp/ballmapper-sn-simples/bm-${bmCmpInv}.edge.out`,
              ),
            ),
            fetch(
              staticify(
                `/static/tmp/ballmapper-sn-simples/bm-${bmCmpInv}.pcbl.out`,
              ),
            ),
          ])
            .then((res) => Promise.all(res.map((r) => r.text())))
    ...
    <Container>
      ...
      <div>
        ...
        <ul style={{ margin: "0", marginBottom: "1em" }}>
          {/* <li>Everything is A1 up to A9 unless otherwise indicated.</li> */}
          <li>
            Ballmappers have <TeX>\epsilon = 0.3</TeX> and either have
            coefficients scaled with 3rd root or 5th roots.
          </li>
          <li>Everything is A1 up to A9 unless otherwise indicated.</li>
        </ul>
        <Typography variant="body1">
        ...
        <ul style={{ margin: "0", marginBottom: "1em" }}>
          <li>
            Size of nodes (barely) indicates how many partitions are inside.
          </li>
          <li>
            Hover over nodes to see their indexing and coverage (for comparison
            ballmapper). To see which permutations live inside each node, see [
